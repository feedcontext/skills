...
- **Newspaper Briefing**: See `briefing-page.md` for editorial shape and prose
  ...
  editorial grid with a masthead, figures, pull quotes, DSL-provided rich-text
  Evidence Links, and topic-scoped `body_units`. The HTML marks this
  reader-facing format as `magazine`.
...
Use `body_units[].rich_text` for inline Evidence Links. The agent chooses the
natural-language anchor text inside the DSL; renderers must not guess which
words to link from `evidence_refs`. Keep `evidence_refs` for provenance and
validation. If a module has no `body_units`, its short `text` fallback is
rendered as-is; web SSR will not expand it or create source controls. Follow
`briefing-page.md` for editorial voice.
  ...
  "body_units": [
    {
      "role": "lead_paragraph",
      "rich_text": [
        {"text": "Central-bank liquidity, social financing, lending, and "},
        {"text": "reverse-repurchase updates", "evidence_ref": "feed_item_1"},
        {"text": " clustered into one market signal."}
      ],
...
`rich_text: [{ text, evidence_ref? }]` annotations. Follow
`narrative-briefing.md` for editorial voice.
...
DSL-provided `rich_text` Evidence Links, optional image fields, and source-index
inputs. The server renderer owns HTML, CSS, toggle behavior, and source-index
markup. It must not infer inline citation anchor text from `evidence_refs`.
